...
inscertificate() {
  ymzs() {
    ...
    certificatec_anytls='/root/ygkkkca/cert.crt'
...
anytlsport() {
  green "anytls端口推荐使用这几个2053 2083 2087 2096 8443端口"
  green "如果是自动获取端口的2053 2083 2087 2096 8443端口被占用,请设置其他端口"
...
anytls_port() {                                             #自动生成
  ...
  port_anytls=""                                            # 初始化最终选定的端口
  tried_ports=()                                            # 初始化一个空数组，用于记录已被尝试且被占用的端口
  while true; do                                            # 循环直到找到一个未被占用的端口，或者所有预设端口都已尝试
    if [[ ${#tried_ports[@]} -eq ${#anytls_num[@]} ]]; then # 检查是否所有预设端口都已被尝试
      yellow "自动选择Anylts端口失败，所有预设端口均被占用。"
      anytlsport # 执行 anytlsport() 函数
      # 假设 anytlsport() 函数会设置 port_anytls 或者执行其他逻辑
      # 在这里检查 anytlsport() 是否成功设置了端口
      if [[ -n "$port_anytls" ]]; then
        break # 如果anytlsport()成功设置了端口，则跳出循环
      else
        red "anytlsport() 函数未能提供可用端口，请手动检查或退出脚本。"
        exit 1 # 或者根据需求进行其他错误处理
      fi
    fi
    # 构建当前可供选择的端口列表（排除已尝试的）
    available_ports=()
    for p in "${anytls_num[@]}"; do
      # 检查当前端口是否在已尝试列表中
      is_tried=false
      for tp in "${tried_ports[@]}"; do
        if [[ "$p" == "$tp" ]]; then
          is_tried=true
          break
        fi
      ...
      if ! $is_tried; then
        available_ports+=("$p") # 如果不在已尝试列表中，则加入可用列表
      fi
    done
    # 确保还有可用的端口可以随机选择 (理论上在上面的if条件已覆盖，但多一层防御)
    if [[ ${#available_ports[@]} -eq 0 ]]; then
      # 这种情况理论上不应该发生，因为上面的 if 应该已经捕获了所有端口都被尝试的情况
      # 但作为冗余检查，也可以在这里再次调用 anytlsport()
      yellow "内部错误：可用端口列表为空，但未触发所有端口占用检查。正在尝试备用方案。"
      anytlsport
      if [[ -n "$port_anytls" ]]; then
        break
      else
        red "anytls_port() 函数未能提供可用端口，请手动检查或退出脚本。"
        exit 1
      fi
    fi
    # 从当前可用端口中随机选择一个
    num_tls=${available_ports[$RANDOM % ${#available_ports[@]}]}
    # 检查随机选择的端口是否被 TCP 占用
    if [[ -z $(ss -tunlp | grep -w tcp | awk '{print $5}' | sed 's/.*://g' | grep -w "$num_tls") ]]; then
      ...
      tried_ports+=("$num_tls")
      yellow "端口 $num_tls 已被占用，正在尝试其他端口... (已尝试 ${#tried_ports[@]}/${#anytls_num[@]} 个不同端口)"
    fi
  done
}
name_password() {
...
instsllsingbox() {
  ...
  sbservice
  sbactive
  #curl -sL https://raw.githubusercontent.com/yggmsh/yggmsh123/main/sbox-v | awk -F "更新内容" '{print $1}' | head -n 1 > /etc/s-box/v
  curl -sL https://raw.githubusercontent.com/yggmsh/yggmsh123/main/sbox-v | awk -F "更新内容" '{print $1}' | head -n 1 >/etc/s-box/v
  red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
m12=$(cat /etc/s-box/anytls.txt 2>/dev/null)
...
lnsb() {
  rm -rf /usr/bin/sb
  curl -L -o /usr/bin/sb -# --retry 2 --insecure https://raw.githubusercontent.com/yggmsh/yggmsh123/main/sb.sh
  chmod +x /usr/bin/sb
}
...
unins() {
  ...
  service iptables save >/dev/null 2>&1
  green "Sing-box卸载完成！"
  blue "欢迎继续使用Sing-box-yg脚本：bash <(curl -Ls https://raw.githubusercontent.com/yggmsh/yggmsh123/main/sb.sh)"
  echo
}
...
latestV=$(curl -sL https://raw.githubusercontent.com/yggmsh/yggmsh123/main/sbox-v | awk -F "更新内容" '{print $1}' | head -n 1)
if [ -f /etc/s-box/v ]; then
  if [ "$insV" = "$latestV" ]; then
    ...
    echo -e "${yellow}$(curl -sL https://raw.githubusercontent.com/yggmsh/yggmsh123/main/sbox-v)${plain}"
  fi
else
